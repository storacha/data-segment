  ...
  'aggregate as generate unsealed CID': async (assert) => {
    ...
    const source = [
      ...
      },
      {
        ...
        location: { level: 23, offset: 0x10n },
      },
      {
        node: Node.from([
      ...
      },
      {
        ...
        location: { level: 23, offset: 0x18n },
      },
      {
        node: Node.from([
    ...
    for (const { location, node } of source) {
      const proof = hybrid.collectProof(location.level, location.offset)
